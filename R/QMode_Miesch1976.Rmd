...
principal_factor_score <- function(data) {
  W_ <- t(normalize_and_transform(data))
  #correct results with cabfac_Q but wrong with principal_factor_loadings
  Q  <- cabfac_Q(data)
...
t(principal_factor_score(original_data))
```

These do not match with the values in Miesch at all. They do when using `cabfac_Q` (see above). Something might be wrong in `principal_factor_loadings`, which might be related to using a different normalization.
...
principal_factor_loadings(original_data) %*% principal_factor_score(original_data)
```
